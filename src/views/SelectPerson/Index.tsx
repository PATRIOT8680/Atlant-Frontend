...
import SpawnPlayerTab from "./spawnPlayer";
...
const SelectPerson = ({ personData }: IPersonData) => {
  ...
  const [selectSpawnPlayer, setSelectSpawnPlayer] = useState<boolean>(false)
  const [showSelectPerson, setShowSelectPerson] = useState<boolean>(true)
  const [currentPerson, setcurrentPerson] =useState<any>(true)
  ...
  const handleNewChar = (slot: string) => {
    ...
    mp.trigger('client:events:createNewPlayer', slot)
  ...
  return (
		<>
			<div className={`select-person ${showSelectPerson ? '' : 'hidden'}`}>
        ...
        <div className="main-section">
          <div className='right-block'>
            ...
            { selectPersonStates.onePerson?.rp_name === '' && (
              <div className={`person-act ${selectFreeSlot === 'one' ? 'select' : ''}`} onClick={() => handleNewChar('one')} id="free">
								<div className='line'></div>
								<div className='selection-person'>
            ...
            { selectPersonStates.twoPerson?.rp_name === '' && (
              <div className={`person-act ${selectFreeSlot === 'two' ? 'select' : ''}`} onClick={() => handleNewChar('two')} id="free">
								<div className='line'></div>
								<div className='selection-person'>
                  <span className='left-content'><img src={userSVG} /></span>
                  <div className='right-content'>
          ...
          <button className="select-person-btn" onClick={() => handleClickSelect(selectPerson.rp_name)}>Выбрать персонажа</button>
				</div>
        </div>
      ...
      {selectSpawnPlayer && (<SpawnPlayerTab personData = {currentPerson} name = {selectPerson.rp_name}  setSelectSpawnPlayer = {setSelectSpawnPlayer}/>)}
		</>
	)
}
