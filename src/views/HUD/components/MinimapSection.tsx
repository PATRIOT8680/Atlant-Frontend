...
const MinimapSection = () => {
  ...
  useEffect(() => {
    const hudp = (value: {type: any} ) => {
      dispatch({type: 'SET_WATER', payload: value.type.drink})
      dispatch({type: 'SET_EAT', payload: value.type.eat})
      dispatch({type: 'SET_STREET', payload: value.type.street})
      dispatch({type: 'SET_DISTRICT', payload: value.type.district})
      dispatch({type: 'SET_CASH', payload: value.type.wallet})
      value.type.microphone ? dispatch({type: 'MICROPHONE_ENABLE'}) : dispatch({type: 'MICROPHONE_DISABLE'})
    }
    EventManager.addHandler('hudp', hudp);
    return () => EventManager.removeHandler( 'hudp', hudp );
  }, [])
